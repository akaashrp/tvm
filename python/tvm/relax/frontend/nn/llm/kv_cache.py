...
class PagedKVCache(Object):  # pylint: disable=too-few-public-methods
    ...
    def attention_with_q_from_cache(
        self,
        source_layer_id: int,
        q: Tensor,
        current_k: Tensor,
        ...
        """Compute decoder attention using K/V projected by another logical layer.

        ``current_k`` and ``current_v`` are the source layer's K/V for the active chunk. During
        prefill they are combined with the source layer's paged past K/V; during decode the source
        layer has already appended them and the operation reads the cache directly.
        """
        # pylint: disable=protected-access
        b, s, h_qo, d_qk = q._expr.ty.shape
        ...
        return Tensor(
            _expr=rx.BlockBuilder.current().emit(
                rx.call_dps_packed(
                    "vm.builtin.attention_kv_cache_attention_with_q_from_cache",
                    [
                        self._expr,
    ...
    def __init__(  # pylint: disable=too-many-locals
        ...
        mha_functions = (
            [
                ...
                rx.Tuple([rx.StringImm("tirx"), bb.add_func(_attention_prefill(num_key_value_heads, num_attention_heads, qk_head_dim, dtype, True, rope_scaling, target, sliding_window_size=layer_sliding_window_size), "tir_attention_prefill_sliding_window")]),
        ...
        if target.kind.name == "llvm":
            ...
            args.extend(
                [
                    ...
                    rx.Tuple([rx.StringImm("tirx"), bb.add_func(_attention_prefill_cpu(num_key_value_heads, num_attention_heads, qk_head_dim, dtype, True, rope_scaling, sliding_window_size=layer_sliding_window_size), "tir_attention_prefill_cpu_sliding_window")]),
