...
def _has_einsum_buffer_access(block: tirx.SBlock) -> bool:
    """Check the buffer-access restriction required by ``Schedule.pad_einsum``."""
    return all(
...
def is_gemv(sch: s_tir.Schedule, block_info: SBlockInfo) -> list[tirx.Buffer] | None:
    ...
    conditions.append(_has_einsum_buffer_access(block_stmt))
    conditions.append(
